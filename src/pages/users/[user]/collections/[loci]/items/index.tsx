...
import { LociRootData } from '@/pages/api/users/[user]/collections/[loci]/items'
...
export default function ItemsPage() {
  ...
  const itemsPath = ['users', router.query.user, 'collections', router.query.loci, 'items']
    .map((c) => encodeURIComponent(c as string))
    .join('/')

  async function fetchItems(): Promise<Array<NavListItem> | null> {
    const itemData = await getData<LociRootData>(`${BASE_URI}/api/${itemsPath}`, getIdTokenClaims)
    if (!itemData) return null
    return itemData.items.map((i) => {
      return {
        text: i.name,
        path: `${BASE_URI}${i.path}`,
      }
    })
  }
  ...
  const handleDeleteItem = async (item: string) => {
    const onConfirmation = async (confirmed: boolean) => {
      if (confirmed) {
        setLoading(true)
        await httpDelete(`${BASE_URI}/api/${itemsPath}/${encodeURIComponent(item)}`, getIdTokenClaims)
        loadItems()
      } else {
